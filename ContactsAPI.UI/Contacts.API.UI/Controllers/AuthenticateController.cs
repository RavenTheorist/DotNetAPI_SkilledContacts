...
namespace Contacts.API.UI.Controllers
{
	...
	public class AuthenticateController : ControllerBase
	{
		...
		[HttpPost]
		[Route("register")]
		public async Task<IActionResult> Register([FromBody] AuthenticateContactDto dtoContact)
		{
			IActionResult result = this.BadRequest();

			IdentityUser user = new IdentityUser(dtoContact.Login);
			user.Email = dtoContact.Login;
			user.UserName = dtoContact.Name;
			var userCreationTask = await _userManager.CreateAsync(user);

			if (userCreationTask.Succeeded)
			{
				dtoContact.Token = this.GenerateJwtToken(user);
				result = this.Ok(dtoContact);
			}
		...
		public async Task<IActionResult> Login([FromBody] AuthenticateContactDto dtoContact)
		{
			IActionResult result = this.BadRequest();

			try
			{
				var user = await this._userManager.FindByEmailAsync(dtoContact.Login);
				if (user != null)
				{
					//var passwordCheckPass = await this._userManager.CheckPasswordAsync(user, dtoContact.Password);
					//if (passwordCheckPass)
					//{
					result = this.Ok(new AuthenticateContactDto()
					{
						Login = user.Email,
						Name = user.UserName,
			...
			{
				this._logger.LogError("Login", exception, dtoContact);
